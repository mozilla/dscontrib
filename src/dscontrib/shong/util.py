...
def read_parquet_from_s3(path, spark=spark):
	"""
...
def write_parquet_to_s3(df, path, mode='overwrite', spark=spark):
	"""
...
def register_df(df, name, cache=False, sqlContext=sqlContext):
	"""
	register df for sparksql
	"""
